...
def _process_prompt(intent_val: str, model_selection: str, action_intent: str, cfg: dict):
    # If the user selected a specific action, we prepend it to the raw prompt to guide the AI
...
def render_workspace(cfg: dict) -> None:
    ...
    with st.container(border=True):
        # Using a two-column layout inside the control box for compactness
        c1, c2 = st.columns(2)
        with c1:
