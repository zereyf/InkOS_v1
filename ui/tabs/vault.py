...
============================================
v8.4: Master Sync — The Athar Protocol.
      - Fixed HTML Leakage (html.escape injection).
      - Hardened Forensic Designations (Upper-case titles).
      - Balanced Mobile Action UI.
"""

import streamlit as st
import textwrap
import html  # 🟢 NEW: Critical for leak protection
...
def render_vault() -> None:
    ...
    # ── 3. ASSET LISTING (ATHAR PROTOCOL + LEAK PROTECTION) ───────────────────
    ...
    for entry in results:
        ...
        # 🟢 SAFETY: Escape strings to prevent layout fractured by HTML orphan tags
        ...
        # 4. Render Expander (Forensic Header)
        with st.expander(f"{indicator} [{entry['id'][:6]}] {safe_title} · {score}% {time_tag}"):
            ...
            aesthetic_badge = f"<span style='background:rgba(201,168,76,0.1); color:var(--gold); padding:2px 6px; border-radius:2px; margin-right:5px;'>{aesthetic.upper()}</span>" if aesthetic and aesthetic != "None" else ""
            ...
            meta_html = textwrap.dedent(f"""
                <div style="background:rgba(255,255,255,0.02); {border_fx} padding:15px; border-radius:3px; margin-bottom:12px; font-family:var(--font-m); font-size:0.55rem; color:var(--text-muted); opacity:{opacity};">
                    <div style="display:flex; justify-content:space-between; margin-bottom:8px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:5px;">
                        <span>TARGET: <b style="color:var(--text);">{entry['target'].upper()}</b></span>
                        <span>AGE: <b style="color:var(--text);">{age_days} DAYS</b></span>
                    </div>
                    <div style="margin-bottom:10px;">
                        <span>FRAMEWORK: <b style="color:var(--text);">{entry['framework'].upper()}</b></span>
                    </div>
                    <div style="background:rgba(201,168,76,0.05); padding:8px; border-radius:2px; margin-bottom:10px; color:var(--text-dim); font-style:italic; line-height:1.4;">
                        "{safe_intent}"
                    </div>
                    <div style="display:flex; gap:5px; flex-wrap:wrap;">
                        {aesthetic_badge} {islamic_badge}
                    </div>
                    <div style='margin-top:10px; font-size:0.5rem; color:var(--steel); letter-spacing:1px;'>TAGS: {safe_tags.upper()}</div>
                </div>
            """)
            ...
            with a1:
                if st.button("⚡ DEPLOY TO WORKSPACE", key=f"dep_{entry['id']}", use_container_width=True):
                    # Synchronized sync with ta_input_widget to preserve real-time load HUD
