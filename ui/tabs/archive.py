...
===================================================
v2026.4.24: Final Behavioral HUD — Tone & Persona Sync.
           - ADDED: 6-Cell High-Density Telemetry Grid.
           - ADDED: Persona Icon sync to Expander labels.
           - ADDED: Tone Radar & Visual DNA (Color Chips) HUD.
           - FIXED: StreamlitAPIException via Callback Protocol.
           - AESTHETIC: Chamfered Glass HUD & Gold-Leaf Typography.
"""
...
def _handle_rehydrate(target, framework, aesthetic, mission_id):
    ...
    # Store toast info in session state because toast won't show during rerun
...
def render_archive() -> None:
    ...
    with f2: fp = st.selectbox("Tone Radar", ["All"] + all_tones, key="arc_tone_ft", label_visibility="collapsed")
    ...
    filtered = [
        ...
        and (fp == "All" or e.get("tone", "NEUTRAL") == fp)
    ]
    ...
    for item in reversed(filtered):
        ...
        label_html = f"""
            <div style="display:flex; justify-content:space-between; width:100%; font-family:var(--font-m); font-size:0.7rem; letter-spacing:1px;">
                <span><span style="color:{dot_color}; margin-right:10px;">●</span> {p_icon} [{item['id']}] {item['time']}</span>
        ...
        with st.expander(label_html, expanded=False):
            ...
            # ── 🔵 HIGH-DENSITY 6-CELL TELEMETRY HUD ──
            ...
            ">
                ...
                <div style="background:rgba(10,12,16,0.6); padding:10px 5px; text-align:center; border-right:1px solid rgba(255,255,255,0.05);">
                    <div style="font-size:0.45rem; color:var(--text-dim); margin-bottom:4px;">TONE_RADAR</div>
