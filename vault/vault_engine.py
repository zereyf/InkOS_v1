...
vault/vault_engine.py — Prompt Memory Vault Engine
====================================================
v18.3: Master Sync — Telemetry Alignment.
       FIXED: Replaced all 'sb' instances with 'supabase'.
       STABLE: Identity rehydration and vault operations restored.
"""
...
def check_id_availability(user_hash: str) -> Tuple[bool, str]:
    ...
    try:
        # 🟢 UPDATED: sb -> supabase
        res = supabase.table(TABLE_USERS).select("id").eq("id", user_hash.strip()).execute()
        return (len(res.data) == 0, f"ID '{user_hash}' is {'available' if len(res.data)==0 else 'taken'}.")
    except Exception as e:
        return False, f"Check failed: {str(e)}"
...
def rehydrate_session(user_hash: str) -> dict:
    """
    Recovers the entire terminal state from Supabase to survive refreshes.
    Returns: {personas: [], dna: {ink, intel, hikmah}}
    """
    state = {
        "personas": [],
    ...
    try:
        # 1. Recover Personas (sb -> supabase)
        p_res = supabase.table(TABLE_PERSONAS).select("*").eq("user_hash", user_hash).execute()
        state["personas"] = p_res.data or []
        
        # 2. Recover DNA Strings from User Profile (sb -> supabase)
        u_res = supabase.table(TABLE_USERS).select("ink_dna, intel_dna, hikmah_dna").eq("id", user_hash).execute()
        if u_res.data:
            profile = u_res.data[0]
            state["dna"] = {
                "ink": profile.get("ink_dna", ""),
...
def search_vault(user_hash: str, **filters) -> Tuple[List[dict], Optional[str]]:
    ...
    try:
        # 🟢 UPDATED: sb -> supabase
        q = supabase.table(TABLE_VAULT).select("*").eq("user_hash", user_hash).order("created_at", desc=True)
        if filters.get("tag_filter"): q = q.ilike("tags", f"%{filters['tag_filter'].lower()}%")
        if filters.get("target_filter") and filters["target_filter"] != "All":
            q = q.eq("target", filters["target_filter"])
...
def delete_prompt(user_hash: str, record_id: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        # 🟢 UPDATED: sb -> supabase
        supabase.table(TABLE_VAULT).delete().eq("id", record_id).eq("user_hash", user_hash).execute()
        return True, None
    except Exception as e:
        return False, str(e)
...
def get_vault_stats(user_hash: str) -> Tuple[dict, Optional[str]]:
    ...
    try:
        # 🟢 UPDATED: sb -> supabase
        res = supabase.table(TABLE_VAULT).select("score, target, tags").eq("user_hash", user_hash).execute()
        if not res.data: return {"count": 0, "avg_score": 0, "top_target": "—", "top_tag": "—"}, None
...
def get_all_tags(user_hash: str) -> List[str]:
    # 🟢 UPDATED: sb -> supabase
    if SUPABASE_MISSING or supabase is None: return []
    try:
        res = supabase.table(TABLE_VAULT).select("tags").eq("user_hash", user_hash).execute()
