...
================================================
v9.4.2: Persona & Cognitive Sync Build.
      - Integrated Cognitive Map v16.3 (Negation/Constraints).
      - Integrated Persona Engine v8.0 (AIZEN Core).
      - Surgical prompt assembly logic.
"""
...
def _call_evaluator(original_input: str, target: str, refined_prompt: str) -> dict:
    ...
    try:
        ...
        if '429' in str(e): return {'score': 0, 'critique': 'Evaluator rate-limited.'}
...
def _call_cipher(system_prompt: str, user_text: str) -> Tuple[Optional[str], Optional[dict], Optional[str]]:
    ...
    try:
        ...
        err_str = str(e)
        if "429" in err_str: return None, None, "[THROTTLED]: Uplink busy."
...
def _build_system_prompt(target, framework, lang, cognitive_directive, persona, islamic_mode, retry_critique) -> str:
    ...
    if cognitive_directive:
        parts.append(f'COGNITIVE STRATEGY:\n{cognitive_directive}')
    if persona:
        # 🟢 SYNCED: Calls new target-aware inject_persona
...
def run_refinement_and_audit(
    ...
    if lang == "Arabic (العربية)":
        # 🟢 SYNCED: Extracts pattern dict from v16.3 Cognitive Map
        pattern_data = detect_arabic_pattern(user_text)
        if pattern_data:
            cognitive = f"[STRATEGY: {pattern_data['pattern']}] {pattern_data['prompt_instruction']}"
        else:
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        if error: return f"Error: {error}", {"score":0, "critique":error}, None
        ...
        if "[CLARIFICATION_REQUIRED]" in refined:
            return refined, {"score": 0, "critique": "Clarification required."}, pattern_data
        audit = _call_evaluator(user_text, target, refined)
        if "rate-limited" in audit['critique'] and best_refined:
            return best_refined, best_audit, pattern_data
